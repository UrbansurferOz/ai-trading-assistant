...
# Execute the Safe Stable Deploy (Version Control + Cloud Build + Sydney Deploy)
./safe_stable_deploy.sh "v3.3: Fixed summary parsing and verified Sydney data store connection"
# 1. Grant the Cloud Run Service Account the 'Viewer' role for the Search Engine
...
# 2. Grant it 'Discovery Engine Editor' to ensure it can generate summaries
...
# Deploy the fix
./safe_stable_deploy.sh "v3.4: Restored IAM permissions and added snippet fallback"
cat << 'EOF' > app.py
import streamlit as st
from google.cloud import discoveryengine_v1beta as discoveryengine
import vertexai
...
# Deploy the Vision Upgrade
./safe_stable_deploy.sh "v3.5: Integrated Gemini Vision with TBD Search grounding"
gcloud discoveryengine engines list --location=global
# 1. Assign the Viewer role (to read the data)
gcloud projects add-iam-policy-binding ai-trading-assistant-488403     --member="serviceAccount:521864748129-compute@developer.gserviceaccount.com"     --role="roles/discoveryengine.viewer"
...
# 1. Update requirements.txt with Vertex AI and Google Cloud Auth
...
# 2. Update app.py (Final v3.5 with proper imports)
cat << 'EOF' > app.py
import streamlit as st
from google.cloud import discoveryengine_v1beta as discoveryengine
import vertexai
...
# 3. Push and Deploy
./safe_stable_deploy.sh "v3.5: Fixed ModuleNotFoundError by adding google-cloud-aiplatform"
